 ...
 * 
 */
package org.wlt.expression;

import org.wlt.expression.ExpressionToken.ETokenType;
import org.wlt.expression.datameta.BaseDataMeta;
import org.wlt.expression.datameta.Constant;
import org.wlt.expression.datameta.Reference;
import org.wlt.expression.datameta.Variable;
import org.wlt.expression.format.ExpressionParser;
import org.wlt.expression.format.FormatException;
import org.wlt.expression.function.FunctionExecution;
import org.wlt.expression.op.ConstantEvaluator;
...
public class ExpressionExecutor {
	...
	public List<ExpressionToken> compile(List<ExpressionToken> expTokens)throws IllegalExpressionException{
		...
		for(ExpressionToken expToken : expTokens){
			...
			if(ETokenType.ETOKEN_TYPE_CONSTANT == expToken.getTokenType()){
				//读入一个常量，压入逆波兰式队列
				_RPNExpList.add(expToken);
				//同时压入校验栈
				verifyStack.push(expToken);
				
			} else if (ETokenType.ETOKEN_TYPE_VARIABLE == expToken.getTokenType()){
				//验证变量声明	
			...
			} else if  (ETokenType.ETOKEN_TYPE_OPERATOR == expToken.getTokenType()){
				//读入一个操作符
				if(opStack.empty()){//如果操作栈为空
					...
					while(!opStack.empty() && doPeek){
						...
						//如果栈顶元素是函数,直接将操作符压入栈
						if( ETokenType.ETOKEN_TYPE_FUNCTION == onTopOp.getTokenType() ){
							if(Operator.COLON == expToken.getOperator()){
						...
						}else if( ETokenType.ETOKEN_TYPE_SPLITOR == onTopOp.getTokenType()
						...
						}else if(ETokenType.ETOKEN_TYPE_OPERATOR == onTopOp.getTokenType()){
							//如果栈顶元素是操作符							
			...
			} else if (ETokenType.ETOKEN_TYPE_FUNCTION == expToken.getTokenType()){
				//遇到函数名称，则使用临时变量暂存下来，等待(的来临
				_function = expToken;

			} else if (ETokenType.ETOKEN_TYPE_SPLITOR == expToken.getTokenType()){
				//处理读入的“（”
				...
				//处理读入的“）”	
					...
					while(doPop && !opStack.empty()){						
						...
						if(ETokenType.ETOKEN_TYPE_OPERATOR == onTopOp.getTokenType()){
							if(Operator.QUES == onTopOp.getOperator()){
								//)分割符不可能遇到？，这说明缺少：号
						...
						}else if(ETokenType.ETOKEN_TYPE_FUNCTION == onTopOp.getTokenType()){
							// 如果遇到函数，则说明")"是函数的右括号
							//执行函数校验
		...
		for(ExpressionToken expToken : _RPNExpList){
			...
			if(ETokenType.ETOKEN_TYPE_CONSTANT == expToken.getTokenType()){
				...
				if(varWithValue != null){
					...
					//当变量没有定义时，视为null型
					ExpressionToken constantToken = ExpressionToken.createConstantToken(
							BaseDataMeta.DataType.DATATYPE_NULL
							, null);
					compileStack.push(constantToken);
				}
				...
				for(int i = 0 ; i < opType ; i++){					
					if(!compileStack.empty()){						
						...
						if(ETokenType.ETOKEN_TYPE_CONSTANT == argToken.getTokenType()){
				...
				if(!compileStack.empty()){
					...
					if(")".equals(onTop.getSplitor())){
						...
						while(doPop && !compileStack.empty()){
							...
							if(ETokenType.ETOKEN_TYPE_CONSTANT == parameter.getTokenType()){
	...
	public String tokensToString(List<ExpressionToken> tokenList){
		...
		for(ExpressionToken token : tokenList){
			...
			ETokenType tokenType = token.getTokenType();
	...
	private ExpressionToken verifyOperator(ExpressionToken opToken , Stack<ExpressionToken> verifyStack)throws IllegalExpressionException{
		...
		for(int i = 0 ; i < opType ; i++){			
			if(!verifyStack.empty()){				
				...
				}else if(ETokenType.ETOKEN_TYPE_VARIABLE == argToken.getTokenType()){
	...
	private ExpressionToken verifyFunction(ExpressionToken funtionToken , Stack<ExpressionToken> verifyStack)throws IllegalExpressionException{
		...
		if(!verifyStack.empty()){
			...
			while(doPop && !verifyStack.empty()){
				...
				}else if(ETokenType.ETOKEN_TYPE_VARIABLE == parameter.getTokenType()){
